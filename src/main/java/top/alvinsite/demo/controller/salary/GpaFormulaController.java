...
public class GpaFormulaController {
    ...
    public void save(@Valid SalaryRuleQuery salaryRuleQuery, @RequestBody @Valid List<GpaFormula> gpaFormulas) {

        Assert.notEmpty(gpaFormulas, "请添加规则后再进行保存");

        gpaFormulas.forEach(rule -> rule.setDepartment(salaryRuleQuery.getDepartment()));

        gpaFormulaService.remove(Wrappers.<GpaFormula>lambdaQuery()
                .eq(GpaFormula::getDepartment, salaryRuleQuery.getDepartment())
                .eq(GpaFormula::getYear, salaryRuleQuery.getYear())
        );
